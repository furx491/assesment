...
def _avg_last2(dept, skip='خدمة'):
    """متوسط آخر عمليتين في القسم عبر فوتيهات أريكة (بعد شيل عمليات الخدمة)"""
    v=[]
    for p in _ref:
        l=[o for o in RT[p].get(dept,[]) if skip not in o[1]]
        if len(l)>=2: v.append(l[-2][2]+l[-1][2])
    return sum(v)/len(v) if v else 0.0
STATUS['تورين فوتيه (اريكة)']=dict(qty=8,
    rem={'التفصيل والكسوة':_avg('التفصيل والكسوة'), 'الدهانات':_avg_last2('الدهانات')},
    cur='الدهانات', stage='الدهانات — آخر مرحلتين',
    note='خلصت نجارة وسفنجة | وقت تقديري — متوسط فوتيه (اريكة)')
...
# ٧. ترابيزات الهالو — فاضلها دهانات بس، بتخلص بكرة 8/9
for _p,_q in [('هالو ترابيزة كبيرة',12), ('هالو ترابيزة وسط',1)]:
    STATUS[_p]=dict(qty=_q, rem=None, cur='الدهانات', stage='الدهانات (آخر مرحلة)',
                    note='خلصت قشرة | بتخلص دهانات بكرة 8/9 | وقت تقديري')

# ---- الأوقات الناقصة في ملف المسارات (بأمر المستخدمة) ----
# روتري سرير: كسوته فاضية → متوسط أصعب سرير | تريكس فوتيه: تشطيبه "لا وقت" → أطول فوتيه
_hard_bed=max(((p,sum(x[2] for x in RT[p]['التفصيل والكسوة']))
               for p in RT if 'سرير' in p and RT[p].get('التفصيل والكسوة')), key=lambda x:x[1])
_long_foo=max(((p,sum(x[2] for x in RT[p].get('تشطيب التنجيد',[])))
               for p in RT if ('فوتيه' in p) and RT[p].get('تشطيب التنجيد')), key=lambda x:x[1])
MISSING_TIME={
 ('روتري سرير','التفصيل والكسوة'): (_hard_bed[1], f'متوسط أصعب سرير ({_hard_bed[0]})'),
 ('تريكس فوتيه','تشطيب التنجيد'):  (_long_foo[1], f'متوسط أطول فوتيه ({_long_foo[0]})'),
}

# ---- أوردرات خلصت خالص ----
FINISHED={'اوردين ترابيزة كبيرة (اريكة)',
          'اوردين ترابيزة صغيرة (اريكة)',   # خلصت خالص
          'بيلي تي فيونت (اريكة)'}          # خلصت خالص (بيلي بوفيه لسه شغالة)
